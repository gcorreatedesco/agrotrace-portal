...
2. **Crear Edge Functions** — `invitar-rt` y actualizar `invitar-ong` para que soporten RT asignado explícitamente.
3. **Verificar Edge Function** — regenerar API Key en resend.com, actualizar secret `RESEND_API_KEY` en Supabase, probar formulario de solicitud end-to-end.
4. **Ejecutar trigger** `on_auth_user_created` en Supabase SQL Editor (ver sección 6).
5. **Ejecutar migración `activo` en perfiles** — agregar campo `activo BOOLEAN DEFAULT TRUE` a tabla de RTs.
6. **Personalizar template "Invite user"** en Supabase → Authentication → Email Templates (texto en español en `AgroTrace_Sist_Auth_Por_Solicitud.html`).
...
13. **Revisar grants a `anon` al activar auth real** — `entregas_correcciones` y `material_documentos` tienen `GRANT ... TO anon`; confirmar que ninguna tabla quede accesible sin sesión una vez que haya login.
